...
public class StudyTwitterUtils {
  ...
  public static void showPostTweetDialogAndPostTweet(@NotNull Twitter twitter, @NotNull final TwitterDialogPanel twitterDialogPanel) {
    ApplicationManager.getApplication().invokeLater(() -> {
      ...
      if (builder.showAndGet()) {
        ...
        try {
          URL imageSource = twitterDialogPanel.getMediaSource();
          if (imageSource != null) {
            File
              imageFile = FileUtil
              .createTempFile(FileUtil.getNameWithoutExtension(imageSource.getFile()), FileUtilRt.getExtension(imageSource.getFile()));
            FileUtil.copy(new File(imageSource.toURI()), imageFile);
  ...
  public abstract static class TwitterDialogPanel extends JPanel {
    ...
    /**
     * 
     * @return URl of media should be posted or null if there's nothing to post 
     */
    @Nullable public abstract URL getMediaSource();
