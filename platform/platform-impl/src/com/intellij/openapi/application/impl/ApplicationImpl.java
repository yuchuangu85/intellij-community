...
public class ApplicationImpl extends PlatformComponentManagerImpl implements ApplicationEx {
  ...
  private static final int ourDumpThreadsOnLongWriteActionWaiting = Integer.getInteger("dump.threads.on.long.write.action.waiting", 300);
    ...
    try {
      ...
      try {
        ...
        if (!myLock.writeLock().tryLock()) {
          ...
          if (ourDumpThreadsOnLongWriteActionWaiting > 0 && false) {
            executeOnPooledThread(new Runnable() {
              @Override
              public void run() {
